...
mmGetPlacementInfo <- function(acc_id = NULL, ProjectId = NULL, mm_token = new_token) {
  ...
  for (n in 1:length(ProjectId)){
    ...
    for (i in 1:length(dataRaw$items)){
      dataTemp <- data.frame(t(as.data.frame(unlist(dataRaw$items[[i]][c('id','project_id','order_ids','name','campaign_xid','utm_source','utm_medium','utm_campaign','fact_status')],recursive = T))),row.names = NULL)
