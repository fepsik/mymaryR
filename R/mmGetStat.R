...
mmGetStat <- function(acc_id = NULL, ProjectId = NULL, mm_token = new_token, start_date = NULL, end_date = NULL, date_group = "month", group = "no") {
  ...
  for (n in 1:length(ProjectId)){
    answer <- GET(paste0("https://app.mymarilyn.ru/api/projects/",as.character(ProjectId[n]),"/statistics?start_date=",start_date,"&end_date=",end_date,"&date_grouping=",date_group,"&grouping=",group),
